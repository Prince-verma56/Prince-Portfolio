...
// --- Mathematics & Geometry Core ---
class Face { constructor(public a: number, public b: number, public c: number) {} }
class Vertex {
  public position: vec3;
  public normal: vec3 = vec3.create();
  public uv: vec2 = vec2.create();
  constructor(x: number, y: number, z: number) { this.position = vec3.fromValues(x, y, z); }
}
...
class Geometry {
  ...
  public get lastVertex(): Vertex { return this.vertices[this.vertices.length - 1]; }
  ...
  public get data() { return { vertices: this.vertexData, indices: this.indexData, normals: this.normalData, uvs: this.uvData }; }
  public get vertexData() { return new Float32Array(this.vertices.flatMap(v => Array.from(v.position))); }
  public get normalData() { return new Float32Array(this.vertices.flatMap(v => Array.from(v.normal))); }
  public get uvData() { return new Float32Array(this.vertices.flatMap(v => Array.from(v.uv))); }
  public get indexData() { return new Uint16Array(this.faces.flatMap(f => [f.a, f.b, f.c])); }
...
function createProgram(gl: WebGL2RenderingContext, shaderSources: [string, string], transformFeedbackVaryings?: string[] | null, attribLocations?: Record<string, number>): WebGLProgram | null {
  ...
  if (attribLocations) {
    for (const attrib in attribLocations) {
      if (Object.prototype.hasOwnProperty.call(attribLocations, attrib)) gl.bindAttribLocation(program, attribLocations[attrib], attrib);
    }
  }
...
class InfiniteGridMenu {
  ...
  private currentScales!: Float32Array;
  ...
  private init(onInit?: (instance: InfiniteGridMenu) => void): void {
    ...
    this.targetScales = new Float32Array(this.DISC_INSTANCE_COUNT).fill(1.0);
    this.currentScales = new Float32Array(this.DISC_INSTANCE_COUNT).fill(1.0);
  ...
  private initTexture(): void {
    ...
    Promise.all(this.items.map(item => new Promise<HTMLImageElement>(resolve => {
      ...
      images.forEach((img, i) => {
        ...
        // ── FIXED: OBJECT-FIT COVER LOGIC TO PREVENT STRETCHING ──
        ...
        ctx.clip(); // Mask it to the perfect square cell
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items, scale = 1.0 }) => {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index: number) => {
      if (items.length) setActiveItem(items[index % items.length]);
    };
                   ...
                   <div key={skill.name} className="flex flex-col gap-2 md:gap-2.5">
                      ...
                      <div className="w-full h-[3px] md:h-[4px] bg-white/10 rounded-full overflow-hidden shadow-inner">
                         <motion.div
                            ...
                            transition={{ duration: 1.2, delay: 0.1 + (index * 0.1), ease: [0.23, 1, 0.32, 1] }}
                            className="h-full bg-gradient-to-r from-[#f04e00] to-orange-400 rounded-full shadow-[0_0_10px_#f04e00]"
                         />
                      </div>
                   </div>
                ))}
